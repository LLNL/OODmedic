...
def get_curve(dir_name, stypes = ['ODIN']):
    tp, fp = dict(), dict()
    fpr_at_tpr95 = dict()
    for stype in stypes:
        known = np.loadtxt('{}/scores_{}_In.txt'.format(dir_name, stype), delimiter='\n')
        novel = np.loadtxt('{}/scores_{}_Out.txt'.format(dir_name, stype), delimiter='\n')
        known.sort()
        novel.sort()
...
def metric(dir_name, stypes = ['ODIN'], verbose=False):
